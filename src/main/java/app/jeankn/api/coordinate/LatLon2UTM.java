...
class LatLon2UTM
{
    ...
    UTM convertLatLonToUTM(double latitude, double longitude )
    {
        verifyLatLon( latitude, longitude );
        convert( latitude, longitude );

        UTM utm = new UTM();
        utm.longZone = String.format("%02d", longitudeZoneValue);
        utm.latZone = digraphArrayN[ latitudeZoneValue ];
        utm.easting = (int) Math.round( eastingValue);
        utm.northing = (int) Math.round( northingValue);
